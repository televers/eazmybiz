...
| Purchase order | Vendor-facing PO: **Purchase order to (vendor)** (party or one-off), **Bill to** / **Ship to** as **your company** addresses (communication profile and up to **three** saved warehouse / shipping addresses), line items, additional charges, payment term, delivery / Incoterm, delivery period, **Delivery by** date (defaults like quotation validity), mandatory terms + optional notes. Same print/PDF template family as quotation / packing list / challan. **Document date** and draft numbering rules match quotation / packing list / challan. **Issue** counts toward **documents (combined)** quota. **After issue:** may be edited anytime; each save logs **what changed** and **when** on the PO detail screen (not on print/PDF). Print/PDF show **First issued on** and **Last revised at** only. |
...
| Monthly documents (combined) — quotation + packing list + delivery challan + **purchase order**, **issued**, per company | 30 | 500 | Unlimited |
...
**Free — baseline:** 1 company · 2 users · **no** company-admin seats (account owner manages the team) · org logo · **Basic** templates · **30** combined issued documents/mo · **60** gate passes/mo · **60** visitor passes/mo · IST monthly reset · **document numbers:** fixed type prefixes (**QT**, **PO**, **PL**, **DC**, **GP**, **VP**); admin picks **one** annual reset: **1 January**, **1 April**, or a **custom** month/day; numbering for a draft follows that document’s date for **which counter** is used; printed form is **prefix + serial** only (no auto year segment) · customer-facing outputs **include** Powered by eazmybiz (upgrade for watermark-free) · all modules listed above · hosted tenant isolation.

**Pro — everything in Free, plus / changed:** **2** companies · **10** users · **2** company-admin seats (max **1** admin per company) · **deactivate** members · **five** print/PDF document themes (**quotation**, **packing list**, **delivery challan**, **purchase order** share the same set) · **500** combined documents/mo · **500** gate passes/mo · **500** visitor passes/mo · **custom document prefixes** (up to **18** characters per type: letters, digits, **/**, **-**), **series reset** (continuous, 1 Jan, 1 Apr, or custom annual date—counter only; no year injected into the printed number), **dash vs slash** before the serial, and optional **multiple numbering series** (**up to 3**, toggled by **company admin or account owner**): each document type is assigned to **series 1–N**; **Series 1** uses the main prefix fields; **Series 2+** may set **optional printed prefix overrides** per document type (same validation as main prefixes) so teams or departments can distinguish numbers on paper; each series has its own counter reset rules; **new** quotation / packing list / challan / **purchase order** screens show a **preview** of the next document number and a **series** dropdown when multi-series is on; once a series has issued numbers, its reset rules are **locked** · **watermark-free** customer-facing outputs.

**Max — everything in Pro, plus / changed:** **5** companies · **50** users · **5** company-admin seats · **several** admins per company (within seat cap) · **eight** print/PDF document themes for quotation, packing list, delivery challan, and **purchase order** (three additional **Max** themes) · **Unlimited** combined documents/mo · **2,000** gate passes/mo · **2,000** visitor passes/mo · same document numbering as Pro, with **up to 5** independent series when multi-series is enabled.
...
|--------|--------|-------------|
| **Documents (combined)** | **30** / month | **Quotation + packing list + delivery challan + purchase order**, combined |
...
- **Primary rule — quotation, packing list, delivery challan, purchase order (document date):** These use an **organization calendar** — the IANA timezone stored on the organization (`calendar_time_zone`) or, when unset, a default derived from the organization **country** (e.g. India → `Asia/Kolkata`). **“Today”**, **backdate limits** (7 / 30 days by role, unchanged), and **blocking future dates** (prepare and issue) are evaluated in that zone so **remote users are not penalized** by their own browser timezone.
