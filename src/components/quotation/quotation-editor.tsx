...
export function QuotationEditor({
  ...
  /** Organization-calendar bounds for quotation date (see document-date-backdate-policy). */
  documentDateMinYmd: string;
  documentDateMaxYmd: string;
  plan: PlanTier;
  /** Draft quotations allow changing the linked party; issued do not. */
  ...
  const issuedQuotationBaseline = useMemo(() => {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!validUntil.trim()) {
      setError("Quotation validity date is required.");
      return;
    }
    if (!billTo.name.trim()) {
      setError("Customer (billing address) name is required.");
      return;
    }
    if (
      ...
      setError("Enter a party name to save the billing address.");
      return;
    }
    ...
    if (mode === "edit" && documentStatus === "issued" && issuedQuotationBaseline) {
      ...
      const bullets = quotationIssuedEditSummaryLines({
        currency,
        baselineLines: issuedQuotationBaseline.lines,
        baselineCharges: issuedQuotationBaseline.charges,
        currentLines: curLines,
        currentCharges: curCharges,
      });
      if (bullets.length > 0) {
        const msg = `Save changes to this issued quotation?\n\n${bullets.map((b) => `• ${b}`).join("\n")}`;
    ...
    try {
      ...
      if (mode === "create") {
        ...
        router.replace(`/quotations/${res.id}`);
      } else if (quotationId) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-8">
      ...
      <div className="space-y-2">
        <PartyFields
          title="Billing Address (customer)"
          value={billTo}
          onChange={setBillTo}
          parties={parties}
          pickTarget="bill"
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      {isNewItemLine ? (
                        <label className="flex items-center gap-2 text-[11px] text-[var(--muted)]">
                          ...
                          />
                          Save as item in database when saving this quotation
                        </label>
                      ) : null}
                    </div>
                  </td>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-[var(--foreground)] font-medium">Quotation valid until *</span>
          <input
            type="date"
            required
          ...
          <span className="text-xs text-[var(--muted)]">
            ...
            Change if you need a different validity.
          </span>
        </label>
      </div>
      ...
      <label className="flex flex-col gap-1 text-sm">
        <span className="text-[var(--muted)]">Any special note (optional) to be printed on quotation</span>
