...
import { QuotationPdfDocument } from "@/lib/quotation/quotation-pdf-document";
import { stampQuotationPdfFooter } from "@/lib/quotation/stamp-quotation-pdf-footer";
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: row, error: rowErr } = await supabase.from("quotations").select("*").eq("id", id).maybeSingle();
  ...
  if (row.status !== "issued") {
    return NextResponse.json({ error: "PDF is available only after the quotation is issued." }, { status: 403 });
  }
  ...
  const raw = await renderToBuffer(
    <QuotationPdfDocument
      ...
      notes={row.notes}
      quotationTerms={orgRow.quotation_terms ?? null}
      logoUrl={logoUrl}
  ...
  const buffer = await stampQuotationPdfFooter(new Uint8Array(raw), {
    docNumber: row.doc_number,
    poweredBy: powered,
  });
