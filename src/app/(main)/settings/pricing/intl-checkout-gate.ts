...
/**
 * Shared eligibility for international (USD) Razorpay checkout (quote + create order).
 */
export async function requireIntlCheckoutContext(targetPlan: "pro" | "max"): Promise<
...
> {
  ...
  if (ctx.organization.commercial_region !== "intl") {
    return { ok: false, message: "International (USD) checkout is only available for non-India-priced accounts." };
  }
