...
import { vendorToFromJson, linesFromJson, additionalChargesFromJson } from "@/lib/purchase-order/parse";
...
export default async function PurchaseOrdersPage() {
  ...
  const { data: rows } = await supabase
    ...
    .select(
      "id, doc_number, status, document_date, currency, issued_at, created_at, vendor_to, lines, additional_charges, created_by_display_name",
    )
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--muted)]">
            Create and organize purchase orders to your vendors in one place
          </p>
        </div>
        <Link href="/purchase-orders/new" className={primaryButtonMd}>
          New purchase order
        </Link>
      </div>
      ...
      <div className={documentListTableCardClassName}>
        <div className={documentListTableScrollAreaClassName}>
          ...
          <tbody>
            {(rows ?? []).map((r) => {
              const vendorName = vendorToFromJson(r.vendor_to).name.trim() || "—";
              ...
              return (
                <tr key={r.id} className="border-t border-[var(--border)]">
                  <td className="px-4 py-3">
                    <Link href={`/purchase-orders/${r.id}`} className="text-sky-600 underline">
                      {r.doc_number}
                    </Link>
                  </td>
