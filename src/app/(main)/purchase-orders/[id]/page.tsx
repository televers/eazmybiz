...
export default async function PurchaseOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const vendorTo = partyFromJson(row.vendor_to);
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/purchase-orders" className="text-sm text-sky-600 underline">
          ← Purchase orders
        </Link>
        <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <dl className="mt-3 grid gap-1 text-sm sm:grid-cols-2">
              <div>
                <dt className="text-[var(--muted)]">Purchase order date</dt>
          ...
          <div className="flex w-full max-w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:justify-end">
            ...
            {row.status === "draft" ? (
              <>
                ...
                <DeleteDraftSalesDocumentButton kind="purchase_order" documentId={id} />
              </>
            ) : null}
          </div>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4 text-sm">
          ...
          <pre className="mt-2 whitespace-pre-wrap font-sans">{formatPartyBlock(vendorTo).join("\n") || "—"}</pre>
        </div>
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4 text-sm">
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4 text-sm">
          ...
          <pre className="mt-2 whitespace-pre-wrap font-sans">{formatPartyBlock(shipTo).join("\n") || "—"}</pre>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 text-sm">
        ...
        <div>
          <div className="font-medium text-[var(--muted)]">Delivery by</div>
          <p className="mt-1 whitespace-pre-wrap">
