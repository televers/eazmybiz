...
export type QuotationPdfFooterStampInput = {
  docNumber: string;
  poweredBy: string | null;
};

/**
 * Stamps quotation no., page x of y (when more than one page), and optional powered-by line
 * onto each page. Uses pdf-lib so the footer is visible regardless of @react-pdf fixed/layer quirks.
 */
export async function stampQuotationPdfFooter(
  pdfBytes: Uint8Array,
  opts: QuotationPdfFooterStampInput,
): Promise<Uint8Array> {
  ...
  for (let i = 0; i < totalPages; i++) {
    ...
    if (totalPages > 1) {
      const parts = [`Quotation No.: ${opts.docNumber}`, `Page ${pageNum} of ${totalPages}`];
