...
import { defaultQuotationTerms, type PackingListTemplateId, type PartySnapshot } from "@/lib/packing/types";
...
function buildQuotationPdfStyles(theme: ReturnType<typeof packingListPdfTheme>) {
...
export function QuotationPdfDocument(props: QuotationPdfInput) {
  const palette = packingListPdfTheme(props.template);
  const styles = buildQuotationPdfStyles(palette);
      ...
      <Page size="A4" orientation="portrait" style={styles.page}>
        ...
        <View style={styles.twoCol}>
          <View style={styles.box}>
            <Text style={styles.boxTitle}>Bill to (customer)</Text>
        ...
        {(props.quotationTerms?.trim() || defaultQuotationTerms())
          .split("\n")
          .filter(Boolean)
          .map((line, i) => (
