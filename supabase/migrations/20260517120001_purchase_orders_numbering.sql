-- Purchase order doc type (po) in next_document_number + peek_document_number

-- Optional printed-prefix overrides per numbering slot (Series 2..N) when multi-series is on.
-- Shape: { "qt": { "2": "RDM/PUBLIC/QT" }, "pl": { "3": "X" }, ... }. Series 1 uses doc_prefix_* only.

alter table public.organizations
  add column if not exists doc_prefix_overrides jsonb not null default '{}'::jsonb;
...
begin
  ...
  v_prefix := case v_type
    ...
    when 'po' then nullif(trim(v_org.doc_prefix_purchase_order), '')
    else null
  end;
  ...
  if v_plan <> 'free'
     and coalesce(v_org.doc_multi_series_enabled, false)
     and v_slot > 1
  then
    v_ov := nullif(trim(coalesce(
      (coalesce(v_org.doc_prefix_overrides, '{}'::jsonb) -> v_type ->> (v_slot::text)),
      ''
    )), '');
